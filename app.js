...
document.addEventListener("keypress", () => {
  // keypress is used to start the game when a user press a key on document
  if (started == false) {
...
function gameFlash(btn) {
  // gameFlash works when user press a button it will flash white
  btn.classList.add("gameflash");
  setTimeout(() => {
    // Here setTimeout is used to flash the button in fraction of seconds
    btn.classList.remove("gameflash");
  }, 200);
}

function userFlash(btn) {
  // userFlash works when user press a button it will flash green
